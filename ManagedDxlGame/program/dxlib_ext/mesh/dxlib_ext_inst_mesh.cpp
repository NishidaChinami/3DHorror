...
namespace dxe {
    ...
    typedef struct _CBUFFER
    {
        ...
        DxLib::COLOR_F diffuse_ = {0, 0, 0};
        DxLib::COLOR_F ambient_ = {0, 0, 0};
        DxLib::COLOR_F emissive_ = {0, 0, 0};
        DxLib::COLOR_F specular_ = {0, 0, 0};
    ...
    InstMeshPool::InstMeshPool(
        ...
        bd.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
        bd.ByteWidth = sizeof(INST_MESH_POOL_CBUFFER);
    ...
    void InstMeshPool::render(Shared<dxe::Camera> camera) {
        ...
        // 
        {
            ...
            if (SUCCEEDED(pImmediateContext->Map(cb, 0, D3D11_MAP_WRITE_DISCARD, 0, &MappedResource))) {
                memcpy(MappedResource.pData, &sb, sizeof(INST_MESH_POOL_CBUFFER));
